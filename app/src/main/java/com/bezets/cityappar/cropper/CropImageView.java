package com.bezets.cityappar.cropper;
...
public class CropImageView extends FrameLayout {
    ...
    public void setImageUriAsync(Uri uri) {
        if (uri != null) {
            ...
            if (currentTask != null) {
                // cancel previous loading (no_image check if the same URI because camera URI can be the same for different images)
                currentTask.cancel(true);
            }

            // either no_image existing task is working or we canceled it, need to load new URI
            clearImageInt();
            mCropOverlayView.setInitialCropWindowRect(null);
     ...
     * the requested width and height info_window_arrow_down-sampling if possible - optimization to get best size to quality.<br>
     * The result will be invoked to listener set by {@link #setOnGetCroppedImageCompleteListener(OnGetCroppedImageCompleteListener)}.
     *
     * @param reqWidth the width to resize the cropped image to (see options)
     * @param reqHeight the height to resize the cropped image to (see options)
    ...
    /**
     * Set visibility of crop overlay to hide it when there is no_image image or specificly set by client.
     */
    ...
    public enum ScaleType {
        ...
        /**
         * Center the image in the view, but perform no_image scaling.<br>
    ...
    public static class CropResult {
         ...
         * Null if save cropped image was executed, no_image output requested or failure.
         */
        private final Bitmap mBitmap;

        /**
         * The Android uri of the saved cropped image result.<br>
         * Null if get cropped image was executed, no_image output requested or failure.
         */
